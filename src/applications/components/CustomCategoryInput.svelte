<script>
	...
	$: customCategoryNames = $currentCustomCategories.map(category => typeof category === "string" ? category : category.name).filter(Boolean);
...
<div class="item-piles-flexrow" style="align-items: center;">
	...
	{#if customCategoryNames.length}
		<datalist id={"item-editor-list-"+id}>
			{#each customCategoryNames as category}
